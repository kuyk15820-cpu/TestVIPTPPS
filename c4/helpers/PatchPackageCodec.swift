...
enum PatchPackageCodec {
    private static let magic = Data("c4PATCH\0".utf8)
    ...
    private static func keyAAD(for packageID: UUID, version: Int) -> Data {
        Data("c4PATCH/v\(version)/key/\(packageID.uuidString)".utf8)
    }

    private static func payloadAAD(for packageID: UUID, version: Int) -> Data {
        Data("c4PATCH/v\(version)/payload/\(packageID.uuidString)".utf8)
    }
}
