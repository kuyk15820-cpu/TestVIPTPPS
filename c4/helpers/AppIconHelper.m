    ...
    dispatch_once(&onceToken, ^{
        ...
        for (size_t i = 0; i < sizeof(candidates) / sizeof(candidates[0]); i++) {
            if (dlopen(candidates[i], RTLD_LAZY | RTLD_GLOBAL)) {
                NSLog(@"[c4] ls: loaded %s", candidates[i]);
                return;
            }
        }
        NSLog(@"[c4] ls: CoreServices/MobileCoreServices dlopen failed");
    });
}
...
static NSDictionary *appsFromWorkspace(void) {
    ...
    for (NSString *selectorName in @[@"allApplications", @"allInstalledApplications"]) {
        ...
        NSLog(@"[c4] ls: %@ returned %lu", selectorName,
              (unsigned long)([candidate isKindOfClass:[NSArray class]] ? [candidate count] : 0));
    }
    if (!apps || apps.count == 0) {
        NSLog(@"[c4] ls: workspace enumeration empty");
        return result;
    }
    NSLog(@"[c4] ls: %@ returned %lu proxies", usedSelector, (unsigned long)apps.count);
    ...
    NSLog(@"[c4] ls: extracted %lu apps (%lu with container)",
...
NSDictionary<NSString *, NSDictionary *> *installedAppInfo(void) {
    ...
    NSLog(@"[c4] ls: workspace empty; MobileInstallation=%lu",
