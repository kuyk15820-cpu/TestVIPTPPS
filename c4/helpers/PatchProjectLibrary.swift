...
enum PatchProjectLibrary {
    ...
    static func load(fileManager: FileManager = .default) -> [PatchLibraryItem] {
        ...
        for url in urls where url.pathExtension.lowercased() == "c4" {
            do {
    ...
    static func save(
        ...
        if let existingURL {
            ...
            var candidate = root.appendingPathComponent(baseName).appendingPathExtension("c4")
            var suffix = 2
            while fileManager.fileExists(atPath: candidate.path) {
                candidate = root.appendingPathComponent("\(baseName)-\(suffix)").appendingPathExtension("c4")
                suffix += 1
            }
